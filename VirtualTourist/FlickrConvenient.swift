...
//MARK: - FlickrClient: FlickrConvenient
extension FlickrClient {
    ...
    func getPhotosForPin(pin: Pin, completionHandler: (success: Bool, errorString: String?) -> Void) {
        ...
        //see if we previously received total num of pages for pin
        var pageNumber = 1
        ...
        //GET Request
        taskForGETMethod(nil, parameters: parameters, parseJSON: true) { (JSONResult, error) in
            if let error = error {
                var errorMessage = ""
                switch error.code {
                case 2:
                ...
                if let photosDictionary = JSONResult.valueForKey("photos") as? [String:AnyObject], photosArray = photosDictionary["photo"] as? [[String:AnyObject]], numPages = photosDictionary["pages"] as? Int {
